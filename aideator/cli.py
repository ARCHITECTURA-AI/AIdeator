...
def command_config_init(args: argparse.Namespace) -> None:
    ...
    llm_provider, llm_model, llm_base = llm_map.get(
        llm_choice, ("ollama", "mistral:7b", "http://localhost:11434")
    )
    ...
    if search_provider in ("tavily", "exa"):
        ...
        print()
        searxng_url = (
            input("  SearXNG instance URL [default: http://localhost:8888]: ").strip()
...
def command_validate(args: argparse.Namespace) -> None:
    ...
    # 1. Create Idea
    idea = save_idea(
        Idea(title=title, description=description, target_user="General", context="CLI run")
    )

    # 2. Create Run
    run = save_run(Run(idea_id=idea.idea_id, mode=mode, tier="medium"))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aideator",
        description=(
            "AIdeator - local-first FastAPI service for idea validation and markdown reports"
        ),
    )
