...
def test_card_invalid_score():
    with pytest.raises(ValidationError):
        Card(
            title="Too High", summary="Invalid", description="Score > 100", score=150, type="demand"
        )


def test_report_validation():
    run_id = uuid4()
    card = Card(title="Test", summary="Test", description="Test", score=50, type="viability")
    report = Report(run_id=run_id, cards=[card], artifact_path="docs/test.md")
